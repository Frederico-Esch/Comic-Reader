...
fn open_comic (io: std.Io, file_path: []const u8, allocator: Allocator) !Comic {
...
pub fn main(init: std.process.Init) !void {
    ...
    defer {
        arena.deinit();
        const leaks = debugAllocator.detectLeaks();
        const check = debugAllocator.deinit();

        if (check == .leak) {
            std.debug.print("Leaks {}\n", .{ leaks });
        }
        else {
            std.debug.print("No leaks ({})", .{ leaks });
        }
    }

    var pages = try open_comic(io, "comic.cbz", allocator);
